...
from mtr.models import meta as model_utils
...
def parse_config():
    ...
    parser.add_argument('--no_ckpt', action='store_true', help='ignore saved checkpoints')
...
def main():
    ...
    model = model_utils.MetaModel(config=cfg.MODEL)
    ...
    if args.ckpt is not None:
        ...
        # Only if args.no_ckpt, meaning we want to reset training progress
        if len(ckpt_list) > 0 and args.no_ckpt:
            eval_output_dir = output_dir / 'eval' / 'eval_with_train'
            files_to_clear = ['best_eval_record.txt', 'eval_list_val.txt']
            for f in files_to_clear:
                if os.path.exists(eval_output_dir / f):
                    os.remove(eval_output_dir / f)
        if len(ckpt_list) > 0 and not args.no_ckpt:
    ...
    train_model(
        ...
        test_loader=val_loader if not args.not_eval_with_train else None,
    ...
    if 'SAMPLE_INTERVAL' in cfg.DATA_CONFIG:
    ...
    repeat_eval_ckpt(
        ...
        val_loader, args, eval_output_dir, logger, ckpt_dir,
