...
def eval_one_epoch(cfg, model, dataloader, epoch_id, logger, dist_test=False, save_to_file=False, result_dir=None, logger_iter_interval=50, pickle_name='result.pkl'):
    ...
    if dist_test:
        pred_dicts = common_utils.merge_results_dist(pred_dicts, dataset.total_tracks(), tmpdir=result_dir / 'tmpdir')
    ...
    with open(result_dir / pickle_name, 'wb') as f:
