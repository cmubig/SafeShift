...
def train_one_epoch(model, optimizer, train_loader, accumulated_iter, optim_cfg,
    ...
    for cur_it in range(start_it, total_it_each_epoch):
        ...
        start = time.time()
        # TODO: for our strategy, how to incorporate scenario scoring?
        # Each model's loss function must compute it
    ...
    with tqdm.trange(start_epoch, total_epochs, desc='epochs', dynamic_ncols=True, leave=(rank == 0)) as tbar:
        ...
        for cur_epoch in tbar:
            ...
            if test_loader is not None and (trained_epoch % ckpt_save_interval == 0 or trained_epoch in [1, 2, 4] or trained_epoch > total_epochs - 10):
                ...
                # tb_dict = eval_one_epoch(
                #     cfg, pure_model, train_loader, epoch_id=trained_epoch, logger=logger, dist_test=dist_train,
                #     result_dir=eval_output_dir, save_to_file=False, logger_iter_interval=max(logger_iter_interval // 5, 1)
                # )
                ...
                if cfg.LOCAL_RANK == 0:
                    ...
                    if 'mAP' in tb_dict and cfg.OPTIMIZATION.BEST_METRIC == 'mAP':
                    ...
                    elif 'MissRate' in tb_dict and cfg.OPTIMIZATION.BEST_METRIC == 'MissRate':
                        best_record_file = eval_output_dir / ('best_eval_record.txt')
                        ...
                        with open(best_record_file, 'a') as f:
                            print(f'epoch_{trained_epoch} MissRate {tb_dict["MissRate"]}', file=f)

                        if best_performance == -1 or tb_dict['MissRate'] < float(best_performance):
                            ckpt_name = ckpt_save_dir / 'best_model'
                            save_checkpoint(
                                checkpoint_state(model, epoch=cur_epoch, it=accumulated_iter), filename=ckpt_name,
                            )
                            ...
                            with open(best_record_file, 'a') as f:
                                print(f'best_epoch_{trained_epoch} MissRate {tb_dict["MissRate"]}', file=f)
                        else:
                            with open(best_record_file, 'a') as f:
                                print(f'{best_src_data[-1].strip()}', file=f)
                    elif 'Accuracy' in tb_dict:
                        best_record_file = eval_output_dir / ('best_eval_record.txt')
                        ...
                        with open(best_record_file, 'a') as f:
                            print(f'epoch_{trained_epoch} Accuracy {tb_dict["Accuracy"]}', file=f)

                        if best_performance == -1 or tb_dict['Accuracy'] > float(best_performance):
                            ckpt_name = ckpt_save_dir / 'best_model'
                            save_checkpoint(
                                checkpoint_state(model, epoch=cur_epoch, it=accumulated_iter), filename=ckpt_name,
                            )
                            ...
                            with open(best_record_file, 'a') as f:
                                print(f'best_epoch_{trained_epoch} Accuracy {tb_dict["Accuracy"]}', file=f)
                        else:
                            with open(best_record_file, 'a') as f:
                                print(f'{best_src_data[-1].strip()}', file=f)
                    else:
