...
def parse_config():
    ...
    parser.add_argument('--best_metric', type=str, default='mAP', help='Which metric to use', choices=['mAP', 'MissRate'])
    ...
    if 'BEST_METRIC' not in cfg.OPTIMIZATION:
        cfg.OPTIMIZATION.BEST_METRIC = args.best_metric
...
def main():
    ...
    if args.ckpt is not None:
        ...
        if len(ckpt_list) > 0 and args.no_ckpt:
            ...
            for f in files_to_clear:
                Path(eval_output_dir / f).unlink(missing_ok=True)
    ...
    if 'BOOTSTRAP' in cfg.DATA_CONFIG:
        cfg.DATA_CONFIG.pop('BOOTSTRAP')
