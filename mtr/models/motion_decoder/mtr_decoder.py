...
from mtr.models.utils.common_layers import MLP
...
class MTRDecoder(nn.Module):
    def __init__(self, in_channels, config):
        ...
        if self.fe_weight_input:
            ...
            self.fe_lg = nn.Linear(self.d_model + 16, self.d_model)
    ...
    def get_decoder_loss(self, tb_pre_tag=''):
        ...
        disp_dict = {}
        total_loss = 0
        # TODO: how to incorporate our loss weighting here?
        for layer_idx in range(self.num_decoder_layers):
            if self.use_place_holder:
                raise NotImplementedError
            ...
            loss_weights = self.forward_ret_dict['loss_weights']

            # TODO: regularize dist by adding in collisions here? Or just straight up using collisions...
            if self.use_coll_loss:
                obj_trajs_fut, obj_trajs_fut_mask = self.forward_ret_dict['obj_trajs_future_state'].cuda(), self.forward_ret_dict['obj_trajs_future_mask'].cuda() 
                track_index_to_predict = self.forward_ret_dict['track_index_to_predict'].cuda()
                collision_dists = pred_trajs.unsqueeze(1)[..., :2] - obj_trajs_fut.unsqueeze(2)[..., :2]
                ...
                # For tmp_decent, do the following
                ...
                # Break dependency on gradient here...is that okay?
                collision_dists[~coll_mask] = torch.inf
                # This is regularization parameter, decrease 0.25 to e.g. 0.1 -> smaller effect, 
                # increase to e.g. 2 -> larger effect of coll_loss
                ...
                # we have access to 'dist' here btw
                #center_gt_positive_idx = dist.argmin(dim=-1)  # (num_center_objects)
                # TODO: explore weighing this with dist instead of just blankly overriding it? Both for VRNN and for this
                # coll_loss = 1*coll_loss + dist
                coll_loss = 100*coll_loss + dist
                # if (coll_loss.argmin(dim=-1) != dist.argmin(dim=-1)).sum() > 4:
                #     breakpoint()

                # For tmp_decent, just set coll_loss = coll_loss
                center_gt_positive_idx = coll_loss.argmin(dim=-1)
            ...
            loss_reg_gmm = loss_reg_gmm * loss_weights
            ...
            loss_reg_vel = loss_reg_vel * loss_weights

            # Don't weight the classifier loss, for consistency with other approaches
    ...
    def get_dense_future_prediction_loss(self, tb_pre_tag='', tb_dict=None, disp_dict=None):
        ...
        loss_weights = self.forward_ret_dict['loss_weights']
        loss_reg = loss_reg * loss_weights
    ...
    def forward(self, batch_dict):
        ...
        # Just refine it here
        if self.fe_weight_input:
            fe_scores = input_dict['fe_scores'].to(obj_feature.device)
            fe_feats = self.fe_weight(fe_scores.unsqueeze(-1))
            center_objects_feature = torch.cat([center_objects_feature, fe_feats], dim=-1)
            center_objects_feature = self.fe_lg(center_objects_feature)
        ...
        if 'scores' in input_dict:
            loss_weights = input_dict['scores'].to(obj_feature.device)
        else:
            loss_weights = torch.ones((obj_feature.shape[0],)).to(obj_feature.device)
        self.forward_ret_dict['loss_weights'] = loss_weights
        self.forward_ret_dict['track_index_to_predict'] = input_dict['track_index_to_predict']
