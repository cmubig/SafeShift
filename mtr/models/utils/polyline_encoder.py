...
class PointNetPolylineGRUEncoder(nn.Module):
    def __init__(self, in_channels, hidden_dim, num_layers=3, num_pre_layers=1, out_channels=None):
        super().__init__()
        self.pre_mlps = common_layers.build_mlps(
            c_in=in_channels,
    ...
    def forward(self, polylines, polylines_mask):
        ...
        # Force into N x L x C
        stacked = feature_buffers.reshape(-1, *feature_buffers.shape[-2:])
        _, h = self.rnn(stacked)
        # Eventually do something like ..reshape(*feature_buffers.shape)
        h = h[-1]
        # We are now at (batch_size, num_polylines, C)
        feature_buffers = h.reshape(*feature_buffers.shape[:2], feature_buffers.shape[-1])
