...
def transform_preds_to_waymo_format(pred_dicts, top_k_for_eval=-1, eval_second=8):
    ...
    if num_future_frames == 16:
        sampled_interval = 1
        start_idx = 0
    elif num_future_frames in [30, 50, 80]:
        sampled_interval = 5
        start_idx = 4
    ...
    for scene_idx, val in enumerate(scene2preds.items()):
        ...
        for obj_idx, cur_pred in enumerate(preds_per_scene):
            ...
            batch_pred_trajs[scene_idx, obj_idx] = cur_pred['pred_trajs'][:topK, np.newaxis, start_idx::sampled_interval, :][:, :, :num_frame_to_eval, :]
...
def waymo_evaluation_explore(pred_dicts, top_k=-1, eval_second=8, num_modes_for_eval=6, return_raw = False):
    ...
    pred_score = tf.convert_to_tensor(pred_score, np.float32).cpu()
    pred_trajs = tf.convert_to_tensor(pred_trajectory, np.float32).cpu()
    gt_trajs = tf.convert_to_tensor(gt_infos['gt_trajectory'], np.float32).cpu()
    gt_is_valid = tf.convert_to_tensor(gt_infos['gt_is_valid'], np.bool).cpu()
    pred_gt_indices = tf.convert_to_tensor(gt_infos['pred_gt_indices'], tf.int64).cpu()
    pred_gt_indices_mask = tf.convert_to_tensor(gt_infos['pred_gt_indices_mask'], np.bool).cpu()
    object_type = tf.convert_to_tensor(gt_infos['object_type'], tf.int64).cpu()
    ...
    if return_raw:
        return avg_results, metric_results, metric_names
    return avg_results

def waymo_evaluation_extra(pred_dicts, extra_val=None, top_k=-1, eval_second=8, num_modes_for_eval=6):
    pred_score, pred_trajectory, gt_infos, object_type_cnt_dict = transform_preds_to_waymo_format(
        pred_dicts, top_k_for_eval=top_k, eval_second=eval_second,
    )
    eval_config = _default_metrics_config(eval_second=eval_second, num_modes_for_eval=num_modes_for_eval)
    ...
    result_dict['-------------------------------------------------------------'] = 0
    assert extra_val is not None, 'Collisions must be provided'
    avg_results.update(extra_val)
    result_dict.update(avg_results)

    final_avg_results = {}
    final_nonvehicle_results = {}
    result_format_list = [
        ['Waymo', 'mAP', 'minADE', 'minFDE', 'MissRate', 'Cols@0.25m', 'Cols@0.5m', 'Cols@1.0m', '\n'],
        ['Vehicle', None, None, None, None, None, None, None, '\n'],
        # ['PEDESTRIAN', None, None, None, None, None, '\n'],
        # ['CYCLIST', None, None, None, None, None, '\n'],
        ['NonVehicle', None, None, None, None, None, None, None, '\n'],
        ['All', None, None, None, None, None, None, None, '\n'],
    ]
    #name_to_row = {'VEHICLE': 1, 'PEDESTRIAN': 2, 'CYCLIST': 3, 'NonVehicle': 4, 'Avg': 5}
    name_to_row = {'VEHICLE': 1, 'NonVehicle': 2, 'All': 3}
    name_to_col = {'mAP': 1, 'minADE': 2, 'minFDE': 3, 'MissRate': 4, 
                   'collisions_0.25': 5, 'collisions_0.5': 6, 'collisions_1.0': 7}

    for cur_metric_name in ['minADE', 'minFDE', 'MissRate', 'mAP', 'collisions_0.25', 'collisions_0.5', 'collisions_1.0']:
        final_avg_results[cur_metric_name] = 0
        final_nonvehicle_results[cur_metric_name] = 0
        for cur_name in ['VEHICLE', 'PEDESTRIAN', 'CYCLIST']:
            final_avg_results[cur_metric_name] += avg_results[f'{cur_metric_name} - {cur_name}']
            if cur_name != 'VEHICLE':
                final_nonvehicle_results[cur_metric_name] += avg_results[f'{cur_metric_name} - {cur_name}']
            else:
        ...
        final_nonvehicle_results[cur_metric_name] /= 2
        result_format_list[2][name_to_col[cur_metric_name]] = '%.4f,' % final_nonvehicle_results[cur_metric_name]
        result_format_list[3][name_to_col[cur_metric_name]] = '%.4f,' % final_avg_results[cur_metric_name]

    result_format_str = ' '.join([x.rjust(18) for items in result_format_list for x in items])
